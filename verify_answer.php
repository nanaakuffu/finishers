        ...
        if ($num > 0) {   // user name was found
          ...
          if ($_POST['security_question'] == $rows[0]['security_question']) {
            if ($_POST['answer'] == decrypt_data($rows[0]['answer'])) {
              ...
              // Add user activity
              $add_activity = $db->add_activity($con, 'login_activity', $_POST['user_name'], 'System was able to succesfully verify you as the user with the name: '.$_POST['user_name']);
              ...
              // Add user activity
              $add_activity = $db->add_activity($con, 'login_activity', $_POST['user_name'], 'System was not able to succesfully verify due to an incorrect answer to the security question.');
              $db->close_connection($con);
